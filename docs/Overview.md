# Overview of AccessibilityInsights for Windows
...
- Hierarchy : The entire visible surface of an application is represented as a tree of Elements.
...
AccessibilityInsights.DesktopUI | P{rovide platform-specific (Windows) implementations of platform-agnostic UI abstractions. This includes things like element highlighters.
...
Assembly | Responsibility
--- | ---
AccessibiltiyInsights | Provide the UI for most users. This application is built using WPF.
...
AccessibiltiyAutomations.Automation | Provide a layer that wraps key actions behind a simplified interface. This layer can then be used either from a .NET application or from PowerShell scripts.
